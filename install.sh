...
check_token() {
  ...
  if [ "$VALID" = '"valid":true' ]; then
    ...
    echo -e "${GREEN}AKSES BERHASIL. Token berlaku hingga $EXPIRES_IN.${NC}"
  else
    echo -e "${RED}AKSES GAGAL. Token tidak valid.${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL="https://github.com/DITZZ112/fox/raw/main/stellar.zip"
        ;;
      2)
        THEME_URL="https://github.com/DITZZ112/fox/raw/main/billing.zip"
        ;;
      3)
        THEME_URL="https://github.com/DITZZ112/fox/raw/main/enigma.zip"
        ;;
      x)
        return
        ;;
  ...
  curl -L "$THEME_URL" -o "$(basename "$THEME_URL")"
